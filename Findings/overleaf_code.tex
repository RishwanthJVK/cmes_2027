...
\begin{itemize}
    ...
    \item \textbf{Spatial dynamics of attention:} Scalp region contributions varied between datasets. For Dataset 1, Central--Temporal electrodes provided the strongest standalone representation. For Dataset 2, the inclusion of the extended feature set showed that Frontal (0.76 accuracy) and Central--Temporal (0.75 accuracy) regions provided the most informative spatial configurations under SVM Raw, whereas Posterior electrodes alone were weaker (0.71 accuracy).
\end{itemize}
...
The ablation analysis revealed that the contribution of feature domains and spatial regions varied across datasets. In Dataset~1, isolating Theta features largely preserved baseline performance, while Central--Temporal electrodes provided the strongest standalone spatial representation. In Dataset~2, Alpha emerged as the dominant frequency band, with its omission substantially reducing SVM performance. For spatial regions, under the upgraded feature set, Anterior (Frontal) (0.76 accuracy) and Central--Temporal (0.75 accuracy) regions provided the strongest standalone spatial representation in Dataset 2, while Posterior electrodes alone were weaker (0.71 accuracy). These findings indicate that spectral and spatial information contribute differently depending on the EEG montage and experimental setting. 
...
\begin{table}[t]
\centering
\caption{Statistical comparison of frequency-band power between mind-wandering and focused conditions. $p$-values correspond to the MW minus Focus comparison.}
\label{tab:statistical_comparison}
\resizebox{\linewidth}{!}{%
\begin{tabular}{lrrr|rrr}
\toprule
& \multicolumn{3}{c|}{Dataset 1} & \multicolumn{3}{c}{Dataset 2} \\
Band & Estimate & $p$-value & Decision & Estimate & $p$-value & Decision \\
\midrule
Delta
& 0.0948 & 0.0901 & NS
& 0.0992 & 0.1271 & NS \\
...
Theta
& 0.0816 & 0.1775 & NS
& 0.0488 & $2.90\times10^{-7}$ & \textbf{Significant} \\

Alpha
& -0.1113 & 0.1983 & NS
& 0.1468 & 0.0110 & \textbf{Significant} \\
...
\midrule
Delta & 11.54 & 8.57 \\
Theta & \textbf{32.47} & \textbf{26.02} \\
Alpha & \textbf{27.94} & \textbf{40.60} \\
Beta  & 11.24 & 13.19 \\
Gamma & 12.07 & 11.62 \\
Other & 4.73 & 0.00 \\
\midrule
